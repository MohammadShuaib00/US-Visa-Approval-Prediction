...
from usvisa.entity.artifact_config import DataIngestionArtifact,DataValidationArtifact
from usvisa.entity.entity_config import DataValidationConfig
from usvisa.constant import SCHEMA_FILE_PATH
    ...
    def detect_dataset_drift(self, reference_df: pd.DataFrame, current_df: pd.DataFrame) -> bool:
        try:
            ...
            dataset_drift_metric = next(
                (metric for metric in json_report["metrics"] if metric["metric"] == "DatasetDriftMetric"), 
                None
            )

            if not dataset_drift_metric:
                raise ValueError("DatasetDriftMetric not found in the report")

            result = dataset_drift_metric["result"]
            ...
            logging.error("An error occurred while detecting dataset drift", exc_info=True)
            raise usvisaException(e, sys)
